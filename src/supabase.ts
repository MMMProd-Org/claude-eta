...
function nullableFiniteNumber(value: unknown, max: number): number | null {
  if (value === null || value === undefined) return null;
  return finiteNumber(value, max);
}

function finiteInteger(value: unknown, max: number): number | null {
...
function parseBaselineRecord(value: unknown): BaselineRecord | null {
...
function parseBaselineResponse(value: unknown): BaselineRecord[] | null {
  if (!Array.isArray(value)) return null;
  const records = value.map(parseBaselineRecord).filter((record): record is BaselineRecord => record !== null);
